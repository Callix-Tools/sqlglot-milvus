...
from sqlglot_milvus import HybridSearch, LoadTable, ReleaseTable, SearchArm, SearchParams
...
CANONICAL: list[tuple[str, str, type]] = [
    ...
    ("insert", "INSERT INTO items (embedding, category) VALUES (:emb, :cat)", exp.Insert),
    ...
    ("alter-add-field", "ALTER TABLE items ADD FIELD tag VARCHAR(32)", exp.Alter),
    ("l1-distance", "SELECT id FROM items ORDER BY embedding <+> :q LIMIT 5", exp.Select),
    ("inner-product", "SELECT id FROM items ORDER BY embedding <#> :q LIMIT 5", exp.Select),
]
...
def assert_accepted(sql: str, node_type: type) -> exp.Expression:
    ...
    """
    ast = parse(sql)
    assert not isinstance(ast, exp.Command), f"degraded to an opaque Command: {sql}"
...
def assert_rejected(sql: str, description: str = "", *, highlight: str | None = None) -> ParseError:
...
@pytest.mark.parametrize(
    ...
    ids=["params-after-limit", "hybrid-before-limit", "hybrid-rerank-limit", "both-clauses"],
)
def test_canonical_clause_order_is_accepted(sql):
...
def test_non_canonical_clause_order_is_rejected(sql, description, highlight):
    assert_rejected(sql, description, highlight=highlight)


def test_clause_order_is_enforced_at_the_default_error_level():
    ...
    with pytest.raises(ParseError, match="SEARCH PARAMS must follow LIMIT"):
        sqlglot.parse_one("SELECT id FROM items SEARCH PARAMS (a=1) LIMIT 10", read="milvus")


@pytest.mark.parametrize("level", [ErrorLevel.WARN, ErrorLevel.IGNORE], ids=["warn", "ignore"])
def test_lenient_error_levels_silently_reorder_the_query(level):
    ...
    ast = sqlglot.parse_one(
        "SELECT id FROM items SEARCH PARAMS (a=1) LIMIT 10", read="milvus", error_level=level
    )
    assert ast.sql(dialect="milvus") == "SELECT id FROM items LIMIT 10 SEARCH PARAMS (a=1)"
...
def test_search_clauses_without_limit_are_accepted(sql):
...
def test_limit_inside_a_subquery_does_not_trigger_the_order_check():
...
def test_search_clauses_in_a_subquery_are_scoped_to_that_subquery(sql):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("SELECT id FROM items LIMIT 10 LIMIT 5", "Found multiple 'LIMIT' clauses"),
    ],
    ids=["two-hybrid", "two-search-params", "two-limit-control"],
)
def test_duplicate_clauses_are_rejected(sql, description):
...
def test_second_rerank_is_rejected():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("SELECT id FROM items LIMIT 10 SEARCH PARAMS (ef_search=64", "Expecting )"),
        ("CREATE INDEX i ON items (embedding USING HNSW", "Expected table name"),
        ...
        ("SELECT id FROM items ORDER BY embedding <-> LIMIT 10", "Invalid expression"),
        ("SELECT id FROM items ORDER BY embedding <=> LIMIT 10", "Invalid expression"),
        ("SELECT id FROM items ORDER BY embedding <#> LIMIT 10", "Invalid expression"),
        ("SELECT id FROM items ORDER BY embedding <+> LIMIT 10", "Invalid expression"),
        ("SELECT id FROM items ORDER BY embedding <->", "Required keyword: 'expression' missing"),
        ("SELECT id FROM items ORDER BY <-> :q LIMIT 10", "Required keyword: 'this' missing"),
        # WEIGHT with a non-numeric operand. (A *missing* operand is a known defect -- see below.)
        ("SELECT id FROM items HYBRID SEARCH (e <=> :d WEIGHT 'x') LIMIT 10", "Expecting )"),
        ("SELECT id FROM items HYBRID SEARCH (e <=> :d WEIGHT 0.7 0.3) LIMIT 10", "Expecting )"),
        ...
        ("SELECT id FROM items LIMIT 10 SEARCH PARAMS ()", "Required keyword: 'expressions'"),
        ("SELECT id FROM items HYBRID SEARCH () LIMIT 10", "Required keyword: 'this' missing"),
        ("SELECT id FROM items HYBRID SEARCH LIMIT 10", "Expecting ("),
        ("SELECT id FROM items HYBRID SEARCH (e <=> :d,) LIMIT 10", "Required keyword: 'this'"),
        # SEARCH PARAMS bodies that are not properties.
        ("SELECT id FROM items LIMIT 10 SEARCH PARAMS (a)", "Expecting )"),
        ("SELECT id FROM items LIMIT 10 SEARCH PARAMS (a=)", "Required keyword: 'value'"),
...
def test_malformed_constructs_are_rejected(sql, description):
    assert_rejected(sql, description)


def test_weight_accepts_a_placeholder():
    ...
    ast = assert_accepted(
        "SELECT id FROM items HYBRID SEARCH (e <=> :d WEIGHT :w) LIMIT 10", exp.Select
    )
...
def test_two_or_more_trailing_words_are_rejected(sql):
...
@pytest.mark.parametrize(
    ...
    ids=["alias-taken", "after-limit", "after-search-params", "after-release", "after-load"],
)
def test_a_single_trailing_word_is_rejected_when_no_alias_slot_is_open(sql):
...
def test_no_milvusql_construct_degrades_to_command(sql, node_type):
    ...
    assert not list(ast.find_all(exp.Command)), f"opaque Command inside the AST: {repr(ast)}"


def test_search_clauses_survive_generation():
    ...
    out = ast.sql(dialect="milvus", unsupported_level=ErrorLevel.RAISE)
    assert "HYBRID SEARCH" in out and "SEARCH PARAMS" in out
...
@pytest.mark.parametrize(
    ...
    ids=["table-then-alias", "table-then-alias-hybrid", "projection", "alias-after-table"],
)
def test_d2_casualty_adjacent_word_pairs_are_destroyed(sql):
...
def test_quoting_rescues_the_d2_casualty(sql):
...
@pytest.mark.parametrize(
    ...
    ids=["as-columns", "search-table", "params-table", "release-column", "release-alias"],
)
def test_the_bare_words_remain_ordinary_identifiers(sql):
...
def test_alter_add_field_is_structured():
    # The control: the one supported ALTER really does produce a typed node, not a Command.
    ast = assert_accepted("ALTER TABLE items ADD FIELD tag VARCHAR(32)", exp.Alter)
...
@pytest.mark.parametrize(
    ...
    ids=["drop-field", "drop-column", "alter-column-type", "modify-column", "alter-vector-dim"],
)
@pytest.mark.xfail(
    strict=True,
...
def test_unsupported_alter_operations_must_be_rejected(sql):
    assert_rejected(sql)
...
def test_known_defect_unsupported_alter_operations_are_silently_accepted(sql, shape):
    ...
    if shape == "command":
        ...
        assert [i.name for i in ast.find_all(exp.Identifier)] == ["items"]  # `tag` is gone
...
def test_known_defect_alter_column_type_is_accepted_and_rewritten():
    ...
    assert isinstance(ast, exp.Alter)
    assert (
        ast.sql(dialect="milvus") == "ALTER TABLE items ALTER COLUMN tag SET DATA TYPE VARCHAR(64)"
    )
...
def test_weight_without_a_number_must_be_rejected():
    assert_rejected("SELECT id FROM items HYBRID SEARCH (e <=> :d WEIGHT) LIMIT 10")


def test_known_defect_weight_without_a_number_is_silently_dropped():
    ...
    ast = parse("SELECT id FROM items HYBRID SEARCH (e <=> :d WEIGHT) LIMIT 10")
    assert next(ast.find_all(SearchArm)).args.get("weight") is None
    assert (
        ast.sql(dialect="milvus") == "SELECT id FROM items HYBRID SEARCH (e <=> :d) LIMIT 10"
    )
...
def test_load_without_table_or_data_must_be_rejected():
    assert_rejected("LOAD items")


@pytest.mark.parametrize("sql", ["LOAD items", "LOAD"], ids=["load-name", "load-bare"])
def test_known_defect_load_without_table_degrades_to_command(sql):
...
def test_create_index_with_a_dangling_using_must_be_rejected():
    assert_rejected("CREATE INDEX i ON items (embedding) USING")


def test_known_defect_create_index_drops_a_dangling_using():
...
def test_known_defect_union_search_params_emits_sql_that_cannot_be_reparsed():
    """KNOWN DEFECT: in a UNION, the trailing LIMIT binds to the ``Union`` and SEARCH PARAMS to the
    right-hand ``Select`` -- so the generator emits them in the D5-illegal order and its own output
    no longer parses.

    Input is accepted, generation succeeds, and the result is rejected by this very dialect. That
    is a round-trip corruption, not merely an ordering nit: any pipeline that parses, rewrites and
    re-parses will blow up on the second pass with an error pointing at SQL the user never wrote.
    """
    sql = "SELECT a FROM t LIMIT 1 UNION SELECT b FROM u LIMIT 2 SEARCH PARAMS (x=1)"
    ast = parse(sql)
    assert isinstance(ast, exp.Union)
    assert isinstance(ast.args["limit"], exp.Limit)  # LIMIT 2 is the union's, not the select's
    assert isinstance(ast.expression.args["search_params"], SearchParams)

    out = ast.sql(dialect="milvus", unsupported_level=ErrorLevel.RAISE)
    assert out == "SELECT a FROM t LIMIT 1 UNION SELECT b FROM u SEARCH PARAMS (x=1) LIMIT 2"
    assert_rejected(out, "SEARCH PARAMS must follow LIMIT")
...
def test_accepted_normalizations_reach_a_fixpoint(sql, normalized):
    ...
    assert parse(out).sql(dialect="milvus", unsupported_level=ErrorLevel.RAISE) == normalized
...
def test_null_safe_equality_falls_back_to_unambiguous_spelling():
...
def test_milvus_never_parses_null_safe_equality():
